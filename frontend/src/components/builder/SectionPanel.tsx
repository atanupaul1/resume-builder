...
export default function SectionPanel({ activeSection, onSelect, completionMap, sectionOrder, onReorder }: Props) {
  ...
  return (
    <aside className="w-52 flex-shrink-0 bg-white border-r border-gray-100 flex flex-col h-full select-none">
      <div className="px-4 pt-5 pb-3 border-b border-gray-100">
        <p className="text-[10px] uppercase tracking-widest text-gray-400 font-semibold">Sections</p>
        <p className="text-[10px] text-gray-300 mt-0.5">Drag ⠿ to reorder on resume</p>
      </div>
          ...
          return (
            ...
            >
              ...
              <button
                ...
                className={`w-full flex items-center gap-2 px-3 py-3 text-left transition-all duration-100
                  ${isActive ? "bg-indigo-50 border-r-2 border-indigo-500" : "hover:bg-gray-50 border-r-2 border-transparent"}
                  ${isBeingDragged ? "opacity-30" : "opacity-100"}
                  ${isDropTarget ? "bg-indigo-50/40" : ""}
                `}
              >
                {/* Drag handle — 6-dot grid */}
                <span className="flex-shrink-0 cursor-grab active:cursor-grabbing text-gray-300 hover:text-gray-500 transition-colors mt-0.5">
                ...
                <span className={`flex-shrink-0 ${isActive ? "text-indigo-500" : "text-gray-400"}`}>
                  {meta.icon}
                </span>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <span className="text-[10px] text-gray-400 truncate block">{meta.description}</span>
                </div>
              </button>
